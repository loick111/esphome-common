...
│   └── esp32s3-devkit.yaml # ESP32-S3 DevKit (pins: I2C 8/9, UART 16/17)
...
# Validate after modifying common/boards/esp32s3-devkit.yaml:
...
- Can be mixed into *any* device (if resources permit)
...
packages:
  board: !include ../common/boards/esp32s3-devkit.yaml
...
*   *Cause*: Attempting to validate a partial file directly.
*   *Fix*: Validate an example file that includes it.

**"Secret 'xyz' not defined"**
*   *Cause*: `examples/secrets.yaml` missing required key.
*   *Fix*: Add the key to `examples/secrets.yaml`.

**"Substitution 'abc' not defined"**
*   *Cause*: Instance file doesn't provide substitution needed by a package/board/mixin.
*   *Fix*: Add substitution to instance file's `substitutions:` block or check board preset.

**"Component X requires component Y"**
*   *Cause*: Missing hardware configuration (I2C, UART, pins) in instance or board.
*   *Fix*: Add hardware config (e.g., `i2c:`, `uart:`, `spi:`) to board file or instance.
